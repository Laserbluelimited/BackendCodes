...
from client_mgt.models import CorporateClient,InternetClient
...
def validate_allowed_users_rule(coupon_object, user):
    ...
    if not user in allowed_users_rule.users.all():
        if allowed_users_rule.all_corporate_users and CorporateClient.objects.filter(user=user).exists():
            return True
        elif allowed_users_rule.all_individual_users and InternetClient.objects.filter(user=user).exists():
            return True
        else:
